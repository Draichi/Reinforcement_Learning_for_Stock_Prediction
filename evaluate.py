...
# print(colored('> To  {} ~~> {} {:.7f}\n\n'.format(l, currency.upper(), data[-1]), 'magenta'))	
for t in range(l):
	...
	reward = 0
	print("> {} BTC {:.7f}".format(t, data[t]), end='\r')

	if action == 1: # buy
		agent.inventory.append(data[t])
		print(colored("> {} BTC {:.7f} |".format(t, data[t]), 'cyan'), formatPrice(total_profit), end='\r')
		# print(colored("> BUY {0:.7f}  - Total profit: {}".format(data[t], total_profit), 'cyan'), end='\r')
		...
		# print(colored("> SELL {0:.7f}  - Total profit: {}".format(data[t], total_profit), 'red'), end='\r')
	# elif action == 0:
	# 	print(colored("> HOLD {0:.7f}  - Total profit: {}".format(data[t], total_profit), 'yellow'), end='\r')
